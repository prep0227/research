...
L.append("Table I reports mean hit rate over ten seeds (standard deviations omitted for readability; effect sizes in Table I). "
         "Ours outperforms B0 on **all** trajectory classes and delay profiles by 28--67 percentage points "
         "(all $p<0.001$, $d>1.3$), and outperforms B1 on line, circle, and accel (all $p<0.05$ except circle-fixed/gamma vs B1 marginal in v0.2; "
         "in the final v0.3 configuration circle is significant at $p<0.05$). "
         "On the S trajectory, Ours is not significantly better than B1 ($p>0.05$), an honest limitation discussed in Section VII.\n")
